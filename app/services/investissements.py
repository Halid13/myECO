...
def effort_investissement_mois(db: Session, id_utilisateur: int, today: date | None = None) -> dict:
    """Total versé (apports de capital) ce mois-ci et le mois précédent, tous placements
    de l'utilisateur confondus."""
    today = today or date.today()
    annee_prec, mois_prec = mois_precedent(today.year, today.month)

    versements = (
        db.query(HistoriqueInvestissement)
        .join(Placement)
        .filter(HistoriqueInvestissement.type == "versement", Placement.id_utilisateur == id_utilisateur)
        .all()
    )
...
def evolution_portefeuille(db: Session, id_utilisateur: int, nb_mois: int = 6, today: date | None = None) -> list[dict]:
    ...
    mouvements = (
        ...
        .order_by(HistoriqueInvestissement.date_operation)
        .all()
    )
